...
import src.main.java.br.com.dio.dto.BoardColumnDTO;
...
import static src.main.java.br.com.dio.persistence.entity.BoardColumnKindEnum.findByName;

public record BoardColumnDAO(Connection connection) {
    ...
    public int getNextOrderForBoard(long boardId) throws SQLException {
        var sql = "SELECT COALESCE(MAX(`order`), -1) + 1 AS next_order FROM BOARDS_COLUMNS WHERE board_id = ?";
        try (var statement = connection.prepareStatement(sql)) {
            statement.setLong(1, boardId);
